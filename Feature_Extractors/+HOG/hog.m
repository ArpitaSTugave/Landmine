...
function feature_vector = hog(input)

input=double(input);

[rows, cols] = size(input);
Ix=input; %Basic Matrix assignment
Iy=input; %Basic Matrix assignment

% Gradients in X and Y direction. Iy is the gradient in X direction and Iy
% is the gradient in Y direction
for i = 1:rows-2
    Iy(i, :) = (input(i, :) - input(i+2, :));
end
for i = 1:cols-2
    Ix(:, i) = (input(:, i) - input(:, i+2));
end


angle = atand(Ix./Iy); % Matrix containing the angles of each edge gradient
angle = angle + 90; %Angles in range (0,180)
            ...
            for y = 0:1
                ...
                for p = 1:8
                    for q = 1:8 
                        ...
                        % Binning Process (Bi-Linear Interpolation) to
                        % prevent the effect of aliasing
