...
% feature_set : temporary variable that stores the feature vector generated after applying the suitable feature extraction algorithm
    ...
    for i = 1:numel(Alarms_info)
        Features(i).names = {'ULBP'};

        sig = tuf.get_signature(Alarms_info(i));
        if (isfield(sig, 'channel_index') && sig.channel_index == 3)
            feature_set = ulbp(double(squeeze(sig.GPR(:, params.bkgd_frames, sig.rel_ch))));
        else
            feature_set = ulbp(double(squeeze(sig.GPR(:, sig.rel_ch, params.bkgd_frames)))); % Looking at one channel (given by sig.rel_ch) in all the frames 
        end
        Features(i).vector = feature_set;
    end

str1_1 = '\\ece-bmll-file.ad.ufl.edu\users$\arpita123\Documents\Arpita\TUF\hmds_millbrook_tuf\extracted_features\ULBP'; 
D1 = dir([str1_1, '\*.mat']);
